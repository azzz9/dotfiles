...
A feature map becomes stale when the app changes. This skill is the upkeep
loop for a skill generated by `/create-verification-skill` or any project-local
...
0. **Locate the target.** Find the verification skill in the current runtime's
   documented project skill roots. In this repository, search
   `config/ai/skills/verify-*/`. Several candidates means ask which one. If
   none exists, stop and point at `/create-verification-skill` instead of
   inventing a target.
