---
name: no-comments
description: "Run an independent comment review, fix accepted findings, and offer encodings for claimed constraints."
disable-model-invocation: true
---

# No comments

Run an independent comment review using the current runtime's native reviewer
or worker operation. Act on accepted findings.

Defer to the reviewer's fresh perspective, then make the final scope judgment
...
1. Start one independent reviewer with the current runtime's native
   delegation operation. Pass the scope and do not restate its rules. If the
   runtime has no separate reviewer, inspect the diff in a fresh parent pass.
