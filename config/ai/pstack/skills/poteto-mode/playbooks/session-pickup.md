...
1. Locate the prior trail. Read the active runtime's history path from
   the pstack runtime contract, or use a worker URL or pushed branch when the
   runtime provides one. Read the metadata overview and last messages first,
   then scan back for the decision points. Parse a long history record in a
   worker when possible and keep the reduced timeline in the main thread (the
