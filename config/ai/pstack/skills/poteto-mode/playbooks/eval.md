...
6. **Verify the chain from runtime history, not self-report.** Read each
   candidate's record under the active workspace path from
   the pstack runtime contract. Do not search another runtime's private
   history. Look at which files each candidate actually opened. Grade
