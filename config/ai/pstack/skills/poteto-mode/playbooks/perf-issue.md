...
1. Capture a baseline trace via the repository's real-surface verification method.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation using the configured `perf-issue` role in the runtime contract. Review the diff. Capture a post-fix trace.
