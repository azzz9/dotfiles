...
4. Verify each component against its baseline via image diff on the matching
   surface using the repository's real-surface verification method. A nonzero
   diff is a fail. Investigate the pixel delta. Use the current runtime's
   recurring-work mechanism for repeated checks until the diff is zero.
