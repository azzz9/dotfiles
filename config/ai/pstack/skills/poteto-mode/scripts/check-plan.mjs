...
const LIVE_LANES = "Use the configured lane count and worker model at the PR head";
const SUB_BLOCKS = [
	"Depends on.",
	"Files.",
	"Build.",
...
const PROGRAM_MARKERS = ["persistent objective", "git show origin/main:", /30[- ]minute/, "status message"];
...
for (const pr of prSections) {
	...
	if (live) {
		if (!live.rest.includes(LIVE_LANES)) fail(live.n, `${pr.title}: Verify, live lacks "${LIVE_LANES}"`);
