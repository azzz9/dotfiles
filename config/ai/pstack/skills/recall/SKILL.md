...
repository or `~/.agents/pstack/runtime.md` after installation to locate the
active runtime's history.
Codex and Copilot CLI use different roots and record formats. Do not assume an
editor-specific project slug or a one-message-per-line file. Match the current
workspace before reading any record.
...
3. Fan out across your chat history when the runtime supports it. Give each worker a disjoint history slice and the same return schema. Order candidates by real modification time, inspect the topic before reading full records, and skip the current chat plus obvious noise. Each result names the topic, user's goal, decisions, open threads, struggles, corrections, and artifacts with a runtime record identifier. For one or two records, search directly. Keep raw history out of the main context.
